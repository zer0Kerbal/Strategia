...
namespace Strategia
{
    ...
    public class StrategyNotifier : ScenarioModule
    {
        ...
        IEnumerator<YieldInstruction> FixStrategyText(KSCFacilityContextMenu menu)
        {
            ...
            string currentLevelText = StringBuilderCache.Format("* Max Active Strategies: {0}", currentLevel+1);
            string nextLevelText = StringBuilderCache.Format("<color=#a8ff04>* Max Active Strategies: {0}</color>", currentLevel+2);
